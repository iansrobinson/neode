...
public class SocialNetworkExample
{
    ...
    @Test
    public void buildSocialNetworkBatchInserter() throws Exception
    {
        String SocialNetwork = "socialNetwork.db";
        GraphDatabaseService db = Db.getBatchInserterDB(SocialNetwork);
        DatasetManager dsm = new DatasetManager( db, SysOutLog.INSTANCE );
        ...
        NodeSpecification user = dsm.nodeSpecification( "user", property( "name" ) );
        NodeSpecification topic = dsm.nodeSpecification( "topic", property( "label" ) );
        NodeSpecification company = dsm.nodeSpecification( "company", property( "name" ) );
        NodeSpecification project = dsm.nodeSpecification( "project", property( "name" ) );
        ...
        db.shutdown();
        db = new GraphDatabaseFactory().newEmbeddedDatabase(SocialNetwork);
        GraphStatistics statistics = GraphStatistics.create( db, "Social Network" );
        new AsciiDocFormatter( SysOutLog.INSTANCE ).describe( statistics );
